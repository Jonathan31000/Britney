...
const css = `
  ...
  .mode-toggle { display: flex; gap: 0; border: 1px solid var(--border); border-radius: 6px;
    overflow: hidden; width: fit-content; margin-bottom: 8px; }
  .mode-btn { padding: 8px 20px; font-size: 13px; font-family: inherit; cursor: pointer;
    border: none; background: var(--bg3); color: var(--text3); transition: all .15s; }
...
export default function Parametres() {
  ...
  const handleLaunchScoring = async () => {
    setScoringRunning(true)
    try {
      const qs = new URLSearchParams({ mode: scoring.scoring_mode, reset: true }).toString()
      const r  = await fetch(`${API}/trigger/score?${qs}`, { method: 'POST' })
      const data = await r.json()
      setToast({ ok: data.status === 'ok', text: data.status === 'ok' ? `Scoring terminé : ${data.message}` : `Erreur : ${data.message}` })
    } catch (e) {
  ...
  const fetchConfig = useCallback(async () => {
    ...
    try {
      ...
      setScoring({
        ...
        scoring_mode:            s.scoring_mode?.value ?? 'rules',
      })
    } catch {
  ...
  return (
    <>
      ...
      <div className="params-page">
        ...
        <div className="params-section">
          <div className="params-section-title">
            <span className="params-section-icon">◈</span> Scoring
          </div>

          <div className="params-field">
            <div className="params-label">Mode de scoring</div>
            <div className="mode-toggle">
              <button
                className={`mode-btn ${scoring.scoring_mode === 'rules' ? 'active' : ''}`}
                onClick={() => setScoring(s => ({ ...s, scoring_mode: 'rules' }))}
              >
                ⧖ Règles (sans IA)
              </button>
              <button
                className={`mode-btn ${scoring.scoring_mode === 'ai' ? 'active' : ''}`}
                onClick={() => setScoring(s => ({ ...s, scoring_mode: 'ai' }))}
              >
                ◈ IA Claude
              </button>
            </div>
            <div className="hint">
              {scoring.scoring_mode === 'ai'
                ? 'Nécessite une clé ANTHROPIC_API_KEY valide dans le fichier .env'
                : 'Utilise les mots-clés ci-dessus pour calculer le score — aucune clé API nécessaire'}
            </div>
          </div>
        ...
        <div className="params-actions">
          ...
          <button className="btn btn-ghost" onClick={handleLaunchScoring} disabled={scoringRunning || saving}>
            {scoringRunning ? 'Scoring en cours…' : `◈ Lancer le scoring (${scoring.scoring_mode === 'ai' ? 'IA Claude' : 'règles'})`}
          </button>

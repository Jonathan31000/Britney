...
def login_classic(session: requests.Session) -> bool:
    ...
    try:
        ...
        for inp in soup.find_all("input"):
            name = inp.get("name", "")
            if inp.get("type") == "hidden" or "token" in name.lower() or "csrf" in name.lower():
                csrf = inp.get("value", "")
                break
        ...
        if "connexion" not in resp2.url and resp2.status_code == 200:
            ...
            log_event("piter.at", "login_fail", "login_echoue")
            return False
    ...
    for page_url in get_page_urls(session):
        try:
            ...
            for row in rows:
                ...
                # Seuls les mots-clés exclus bloquent lors du scraping
                excluded = any(
                    kw.lower() in f"{offre.get('titre','')} {offre.get('description','')}".lower()
                ...
                if offre_id:
                    update_statut(offre_id, "filtre_ok")
                    stats["filtre_ok"] += 1
                    stats["nouvelles"] += 1

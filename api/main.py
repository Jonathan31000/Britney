...
@app.get("/api/offres/count", tags=["offres"])
def count_offres(
...
def trigger_score(mode: str = Query("rules", pattern="^(rules|ai)$"), reset: bool = Query(False)):
    try:
        import sys
        sys.path.insert(0, os.path.dirname(os.path.dirname(__file__)))
        if reset:
            ...
            conn.execute("UPDATE offres SET statut='filtre_ok' WHERE statut='scored'")
            conn.commit()
            conn.close()
        from scraper.ai_scorer import run_scorer
        result = run_scorer(force_mode=mode)
        return TriggerOut(status="ok", message=str(result))
    except Exception as e:
        return TriggerOut(status="error", message=str(e))
